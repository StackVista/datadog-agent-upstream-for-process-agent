...
func (pm *ProcessMonitor) Initialize(useEventStream bool) error {
	if useEventStream {
		panic("[STS] we don't implement the event stream feature")
	}
...
func (pm *ProcessMonitor) callExecCallbacks(pid uint32) {
	...
	for c := range pm.processExecCallbacks {
...
func (pm *ProcessMonitor) callExitCallbacks(pid uint32) {
	...
	for c := range pm.processExitCallbacks {
		// Okey, here it goes: golang passes loop variables by reference to
		// closures. We need to assign to a variable first to capture
		cCaptured := *c
		pm.callbackRunner <- func() { cCaptured(pid) }
...
func (pm *ProcessMonitor) startNetlink() error {
	...
	// The done channel is consumed by ProcEventMonitor and written/closed by ProcessMonitor
	...
	// The process monitor will run in its own goroutine
	...
	for notActive := range notRunningPids {
...
func (pm *ProcessMonitor) Stop() {
	...
	// This could happen if we call Stop() before Initialize(). If we don't return here we will try to act on nil channels because they are not yet initialized.
	if !pm.isInitialized {
		pm.m.Unlock()
		return
	}
