...
int BPF_PROG(udp_sendpage_exit, struct sock *sk, struct page *page, int offset, size_t size, int flags, int sent) {
    ...
    return handle_message(&t, sent, 0, CONN_DIRECTION_UNKNOWN, 1, 0, PACKET_COUNT_INCREMENT, sk, pid_tgid);
}
...
static __always_inline int handle_udp_send(struct sock *sk, int sent) {
    ...
    if (sent > 0) {
        ...
        handle_message(t, sent, 0, CONN_DIRECTION_UNKNOWN, 1, 0, PACKET_COUNT_INCREMENT, sk, pid_tgid);
    }
