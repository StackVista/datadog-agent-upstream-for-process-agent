...
/*
Scan the payload (starting at skb_info->data_off) for a literal substring `watch=true`.
Boolean-only match: when found, it returns true.
*/
static __always_inline bool http_find_watch_true_skb(struct __sk_buff* skb, skb_info_t *skb_info) {
  char read_buffer[HTTP_HEADER_READ_BUFFER_SIZE];
  bpf_memset((char*)read_buffer, 0, HTTP_HEADER_READ_BUFFER_SIZE);
