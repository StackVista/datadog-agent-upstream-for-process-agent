...
func ConnectionKeysFromConnectionStats(connectionStats ConnectionStats) []types.ConnectionKey {
	...
	connectionKeys[0] = types.NewConnectionKeyWithNamespace(connectionStats.Source, connectionStats.Dest, connectionStats.SPort, connectionStats.DPort, connectionStats.NetNS)
	connectionKeys[1] = types.NewConnectionKeyWithNamespace(connectionStats.Dest, connectionStats.Source, connectionStats.DPort, connectionStats.SPort, connectionStats.NetNS)
	...
	if hasTranslation {
		...
		connectionKeys[2] = types.NewConnectionKeyWithNamespace(localAddress, remoteAddress, localPort, remotePort, connectionStats.NetNS)
		connectionKeys[3] = types.NewConnectionKeyWithNamespace(remoteAddress, localAddress, remotePort, localPort, connectionStats.NetNS)
	}
...
func WithKey(connectionStats ConnectionStats, f func(key types.ConnectionKey) (stop bool)) {
	...
	if hasNAT && f(types.NewConnectionKeyWithNamespace(clientIPNAT, serverIPNAT, clientPortNAT, serverPortNAT, connectionStats.NetNS)) {
		return
	}

	// Callback 2: (client, server)
	if f(types.NewConnectionKeyWithNamespace(clientIP, serverIP, clientPort, serverPort, connectionStats.NetNS)) {
		return
	}

	// Callback 3: NATed (server, client)
	if hasNAT && f(types.NewConnectionKeyWithNamespace(serverIPNAT, clientIPNAT, serverPortNAT, clientPortNAT, connectionStats.NetNS)) {
		return
	}

	// Callback 4: (server, client)
	f(types.NewConnectionKeyWithNamespace(serverIP, clientIP, serverPort, clientPort, connectionStats.NetNS))
}
